...
class TestJSONPointerEscapes:
    """Test JSON Pointer escape handling in schemas."""
    
    def test_pointer_escapes_in_defs(self, temp_dir):
        """Test that $defs with special characters are handled correctly."""
    ...
    def test_refs_to_escaped_defs(self, temp_dir):
        """Test that $refs to defs with special characters work."""
        schemas_dir = temp_dir / "schemas"
        schemas_dir.mkdir()
...
class TestSLASIntegration:
    """Test SLAS integration with the main lithify pipeline."""
    
    def test_slas_generates_aliases(self, temp_dir):
        """Test that SLAS generates type aliases for scalar types."""
        from lithify.slas_schema_index import SchemaIndex
        from lithify.slas_alias_generator import emit_alias_modules
        from lithify.slas_classifier import classify_shape
        ...
        ref_map, modules = emit_alias_modules(
            index, output_dir, "test_pkg", "BaseModel"
        )
...
class TestInitFileGeneration:
    ...
    def test_generate_init_file(self, temp_dir):
        """Test that generate_init_file creates a proper __init__.py."""
    ...
    def test_generate_init_file_empty_dir(self, temp_dir):
        """Test generate_init_file with empty directory."""
        from lithify.packaging import generate_init_file
...
class TestMetadataFileSuppression:
    """Test that metadata files are not generated."""
    
    def test_no_metadata_files_generated(self, temp_dir, sample_schemas):
        """Test that manifest.json, py.typed, and _slas_ref_map.json are not created."""
        import subprocess
        import sys
        from pathlib import Path
        ...
        cmd = [
            ...
            "--json-out", str(temp_dir / "json"),
            "--models-out", str(temp_dir / "models"),
