...
def _require_cli():
    """Import the CLI and required modules in one place."""
    ...
    def fake_run_generation(cfg, reporter):
        ...
        if cfg.dry_run:
            ...
            return mods.orchestrator.GenerationResult(
                package_dir=cfg.models_out / cfg.package_name,
...
def _stub_formatter_to_noop(monkeypatch):
    """
    Patch formatting.format_path to avoid invoking external tools.
    Returns a list capturing what the CLI attempted to format.
    """
    mods = _require_cli()
    recorded = []
