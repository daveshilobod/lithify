...
def remove_scalar_defs(
    json_dir: Path,
    modules_created: Dict[str, List[str]],
    verbose: int = 0
) -> None:
    ...
    if verbose:
        print(f"[SLAS] Schema preprocessing complete")


def _remove_refs_to_files(node: Any, removed_files: Set[str]) -> bool:
    """Remove $refs pointing to files that no longer exist.
...
def create_slas_placeholder_schema(
    module_name: str,
    types: List[str]
) -> Dict:
