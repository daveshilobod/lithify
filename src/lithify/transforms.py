...
class DocstringWrapper(ast.NodeTransformer):
    """
    Walks an AST and re-wraps class docstrings to a max line length.
    """
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> ast.ClassDef:
        ...
        # Clean up the string to a single line to remove any existing formatting
        cleaned_docstring = ' '.join(docstring.strip().split())
        
        # Only wrap if the single line is longer than our target width
        ...
        # Replace the old docstring node with the new one
...
def wrap_all_docstrings(package_dir: Path, verbose: int = 0) -> None:
    """
    Finds all generated model files and re-wraps their class docstrings.
    """
    changed = 0
    # Use rglob to catch all python files, even in subdirectories
    for py_file in package_dir.rglob("*.py"):
        # Skip __init__, base models, etc.
        ...
        # ast.unparse turns the AST back into code
