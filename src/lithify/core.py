...
def _walk(node: Json) -> None:
    """Recursively walk schema tree and fix float const/enum issues."""
    if isinstance(node, dict):
        ...
        if ("enum" in node and 
            isinstance(node["enum"], list) and 
            len(node["enum"]) == 1 and 
            isinstance(node["enum"][0], float) and 
            not isinstance(node["enum"][0], bool)):
...
def _rewrite_single_ref(ref: str, base_url: Optional[str], schema_map: dict[str, str]) -> str:
    """Rewrite a single $ref from remote to local."""
    original = ref
...
def rewrite_remote_refs(data: Any, schema_map: dict[str, str], base_url: Optional[str]) -> Any:
...
def mirror_yaml_to_json(
    ...
    """
    Convert all .yml/.yaml files under yaml_root to .json under json_root,
    preserving subdirectory structure. Rewrites remote refs if base_url is provided.
    """
    written: list[Path] = []
    ...
    # Copy existing JSON files, skipping ones already in json_root to avoid infinite recursion
...
def _iter_refs(node: Any) -> Iterable[str]:
    """Iterate over all $refs in a schema."""
    if isinstance(node, dict):
        ...
        for ref in _iter_refs(schema):
            ...
            if not candidate.exists():
                ...
                try:
                    ...
                    pass
                problems.append(
                    f"[missing-ref] {jf.relative_to(json_root)}: '{ref}' -> '{rel_target}' not found"
                )
            elif not candidate.is_file():
                problems.append(
                    f"[bad-ref] {jf.relative_to(json_root)}: '{ref}' -> '{candidate}' is not a file"
                )
...
def run_datamodel_codegen(
    ...
    """Run datamodel-code-generator with the validated flag set."""
    ...
    cmd = [
        ...
        "--enum-field-as-literal", "one",  # Literal[...] for single-value enums
